...
export default function RoomTwo({
    ...
    const introLines = ["Hiiii Bubbachii 🥰.. There is something i have been cooking over the last few days for youu 🤪 ", "I bareley held it together without telling you for this long", "I just wanted to make something that feels even a tiny bit as special as you make me feel every single day.", "But more than the surprise itself, I just want you to know how grateful I am for you.", "ilaaaavuuuuuuu bubbaa 💞.. Sooo sooo Muchhh. Hope you enjoy this."];
    useEffect(() => {
...
images.map((img, index) => ( < div key = {
    ...
    } > < div className = "image-wrapper" > < img src = {
        img.src
    }
    alt = ""
    className = "gallery-image" / > < div className = "image-shimmer" > < /div> </div > < p className = "short-text" > {
        img.short
    } < /p> </div > ))
} < /div> {openedCount === 6 && ( <div className="epic-message"> ✨ You unlocked every memory ✨ </div > )
} < /div> {/ * Modal * /} {selectedImage !== null && ( <div className="modal-overlay" onClick={closeModal}> <div className="modal-content" onClick={(e) => e.stopPropagation()} > <button className="close-btn" onClick={closeModal} > ✕ </button > < img src = {
    images[selectedImage].src
}
alt = ""
className = "modal-image" / > < p className = "long-text" > {
